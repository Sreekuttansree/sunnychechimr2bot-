...
async def auto_filter(bot: Client, update: Message):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("𝐍𝐄𝐗𝐓 ⏭️
⏭️", callback_data=f"navigate(0|next|{query})")
                ]
            )
        
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"🔰 𝐏𝐚𝐠𝐞  1/{len_result if len_result < max_pages else max_pages} ⚕️", callback_data="ignore")
        ])
